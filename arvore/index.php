...
<title>Editor de Árvores de Decisão</title>
...
<body>
<h1>Editor de Árvores de Decisão</h1>
<div id="debug"></div>
<?php
switch( $cmd ){
	case 'edit-tree':
...
function showList( $tree ){
	?>
  <a class="btnCreateNewTree">Criar uma nova árvore de decisão &raquo;</a>
  <?php
	$tree->listAll();
...
function showTreeForm( $tree, $selectedRevision ){
	...
	?>
  <p><a href="<?php echo EDITOR_URL; ?>">&laquo; Retornar para a lista</a>&nbsp;&nbsp;&nbsp;
  	<a target="_blank" href="<?php echo VIEWER_URL; ?>?<?php echo $treeViewerID; ?>">Visualizar a árvore em uma nova janela &raquo;</a></p>
  <form id="tree-editor" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
  <?php
	if( !empty( $tree->revisions ) ){
	?>
  <select id="revisions" name="revision">
  	<option value="">Ver Revisões</option>
    <?php
  ...
  <p><label for="description">Descrição:</label><br />
...
function showBranchForm( $tree, $branchID ){
  ...
  <p><label for="content">Questão/Decisão:</label><br />
  ...
  <p id="forks"><label for="fork">Opções:</label><br />
  <?php
  ...
  <?php
	...
	?></p>
  <a href="#" id="btnAddFork">Adicionar Outro</a>
  <p><input type="submit" value="Save" /> 
